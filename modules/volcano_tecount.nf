process VOLCANO_TECOUNT {
    tag "volcano_te_${contrast_name}"
    cpus 2
    memory '8 GB'
    ...
    output:
    path "*_te_volcano.pdf", emit: pdf, optional: true
    ...
    id_col <- intersect(c("tecount", "gene_id"), colnames(res_dt))[1]
    ...
    p <- ggplot(res_dt, aes(x = log2FoldChange, y = -log10(padj), color = sig)) +
        ...
        labs(
            title = paste("TE Volcano Plot:", "${contrast_name}"),
            x = "Log2 Fold Change",
    ...
    pdf("${contrast_name}_te_volcano.pdf", width = 6, height = 5)
    print(p)
    dev.off()
    """
}