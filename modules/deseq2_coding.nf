process DESEQ2_CODING {
    ...
    print("--- Counts Matrix Structure ---")
    str(counts)

    # 2. Retain ONLY ENSEMBL coding genes
    is_ensembl_gene <- grepl("^ENSG|^ENST|^ENSMUSG|^ENSMUST", rownames(counts), ignore.case = TRUE)
    counts <- counts[is_ensembl_gene, , drop = FALSE]

    if (nrow(counts) == 0) {
        stop("No ENSEMBL coding genes (ENSG/ENST/ENSMUSG/ENSMUST) found in the count matrix.")
    }
    ...
    # 4. Dynamic OrgDb annotation detection and Keytype mapping setup
    sample_gene <- rownames(counts)[1]
    if (grepl("^ENSG", sample_gene, ignore.case = TRUE)) {
        library(org.Hs.eg.db)
        org_db <- org.Hs.eg.db
        my_keytype <- "ENSEMBL"
        message("Detected Human ENSEMBL Genes (ENSG). Using org.Hs.eg.db.")
    } else if (grepl("^ENST", sample_gene, ignore.case = TRUE)) {
        library(org.Hs.eg.db)
        org_db <- org.Hs.eg.db
        my_keytype <- "ENSEMBLTRANS"
        message("Detected Human ENSEMBL Transcripts (ENST). Using org.Hs.eg.db.")
    } else if (grepl("^ENSMUSG", sample_gene, ignore.case = TRUE)) {
        library(org.Mm.eg.db)
        org_db <- org.Mm.eg.db
        my_keytype <- "ENSEMBL"
        message("Detected Mouse ENSEMBL Genes (ENSMUSG). Using org.Mm.eg.db.")
    } else if (grepl("^ENSMUST", sample_gene, ignore.case = TRUE)) {
        library(org.Mm.eg.db)
        org_db <- org.Mm.eg.db
        my_keytype <- "ENSEMBLTRANS"
        message("Detected Mouse ENSEMBL Transcripts (ENSMUST). Using org.Mm.eg.db.")
    } else {
        org_db <- NULL
        my_keytype <- NULL
    ...
    for (col_name in candidate_cols) {
        ...
        # PCA export with sample text labels
        pdf(paste0(col_name, "_coding_pca.pdf"))
        vsd <- vst(dds, blind = FALSE)
        pca_data <- plotPCA(vsd, intgroup = "target_factor", returnData = TRUE)
        percentVar <- round(100 * attr(pca_data, "percentVar"))
        
        p_pca <- ggplot(pca_data, aes(x = PC1, y = PC2, color = target_factor)) +
            geom_point(size = 3) +
            geom_text(aes(label = name), vjust = -0.8, size = 3, show.legend = FALSE) +
            xlab(paste0("PC1: ", percentVar[1], "% variance")) +
            ylab(paste0("PC2: ", percentVar[2], "% variance")) +
            theme_minimal() +
            ggtitle(paste("PCA (Coding Genes):", col_name))
