process VOLCANO_IRFINDER {
    tag "volcano_ir_${contrast_name}"
    cpus 2
    memory '8 GB'
    ...
    output:
    path "*_irfinder_volcano.pdf", emit: pdf, optional: true
    ...
    id_col <- intersect(c("gene", "ir_id"), colnames(res_dt))[1]
    ...
    p <- ggplot(res_dt, aes(x = log2FoldChange, y = -log10(padj), color = sig)) +
        ...
        labs(
            title = paste("IRFinder Volcano Plot:", "${contrast_name}"),
            x = "Log2 Fold Change",
    ...
    pdf("${contrast_name}_irfinder_volcano.pdf", width = 6, height = 5)
    print(p)
    dev.off()
    """
}